...
class AltinkaynakConnector:
    ...
    def _get_rate(self, currencies, date, rate_type):
        """
        Currency rate get from altinkaynak.com
        :param currencies: EUR, USD, GBP, etc. (multi)
        ...
        for currency in currencies:
            rate_dict = {
                "AltinkaynakBuying": 1
                / float(soup.find("td", attrs={"id": f"td{currency.upper()}Buy"}).next),
                "AltinkaynakSelling": 1
                / float(
                    soup.find("td", attrs={"id": f"td{currency.upper()}Sell"}).next
                ),
            }
            res[currency] = rate_dict[rate_type]
        return res
